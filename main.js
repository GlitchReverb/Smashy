...
var context;
var player;
var zombiesSprites = new Image();
var soldierSprites = new Image();
var brickSprites = new Image();
var enemies = [];
var bullets = [];
var walls = [];
var inputs = {
...
function init() {
	...
	zombiesSprites.src = 'zombies.png';
	soldierSprites.src = 'soldier.png';
	brickSprites.src = 'brick.png';
	...
	for(var x=0; x<XSIZE; x++) {
		...
		for(var y=0; y<YSIZE; y++){
			if(map.substr(y * XSIZE + x, 1) !== ' '){	
				walls[x][y] = new Entity(x * SIZE, y * SIZE, 'grey');
			}else{
				walls[x][y] = null;
			}
		}
...
function gameLoop() {
	...
	if(inputs.click && coolTimer <= 0){
		...
		var bullet = new Entity(0,0, 'black');
		bullet.size /= 4;
...
/*				context.drawImage(
				brickSprites,
				walls[x][y].x, walls[x][y].y, walls[x][y].size, walls[x][y].size
				);
*/
			}
		}
	...
	context.fillText('score ' + score, canvas.width - 100,40);
